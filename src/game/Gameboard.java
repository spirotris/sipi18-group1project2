...
public class Gameboard {
	...
	private void fixPreviousTile(TileType tiletype) {
	...
	public boolean moveCharacter(Direction direction) {
		...
		//Show the door again after the player moved away from it
		if (isPlayerOnDoor) {
			boardGrid[y][x].setTileType(DOOR);
			isPlayerOnDoor = false;
		}
		if (direction.equals(Direction.RIGHT) || direction.equals(Direction.LEFT)) {
			if (!onCollision(boardGrid[y][x + move])) {
				boardGrid[y][x + move].setTileType(CHARACTER);
				//TODO this removes the door after correcting it
